...
def testModel(model, test_loader, device=None):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            correctS += predS.eq(targetS.argmax(dim=1).view_as(predS)
                                 ).sum().item()
            correctE += predE.eq(targetE.argmax(dim=1).view_as(predS)
                                 ).sum().item()
